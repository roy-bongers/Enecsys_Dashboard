<?php
	...
	$query = mysqli_query($connect,"select date_format(ts, '%d-%m') as day, sum(whtotal) as Wh from enecsys_report where date_format(ts, '%m') = $month and year(ts) = $CurrentYear and id = 0 group by `day`");
	$j = 0;
